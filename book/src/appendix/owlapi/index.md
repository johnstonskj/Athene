...
The [`athene-owlapi`](https://crates.io/crates/athene-owlapi) package is a Rust
OWL API that provides an implementation as close as possible to the *OWL 2 Web
Ontology Language
...
1. Parent types, which are usually abstract, in the OWL 2 specification are
   ...
   1. The `strum` crate provides `is_{variant}` and `try_as_{variant}`
